...
export default function DeleteExpenseForm({ closeModal }: DeleteExpenseForm) {
    ...
    return (
        <>
            <DialogTitle as="h3" className="font-black text-4xl text-purple-950 my-5">Eliminar Gasto</DialogTitle>

            {state.errors.map(error => <ErrorMessage key={error}>{error}</ErrorMessage>)}

            <p className="text-xl font-bold">Confirma para {''}
                <span className="text-amber-500">eliminar</span>
            </p>
